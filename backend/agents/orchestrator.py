...
from core.scoring_engine import Priority, score_routes
...
async def plan_route(
    ...
    priority: Priority = Priority.BEST_TRADEOFF,
    google_maps_api_key: str = "",
    groq_api_key: str = "",
) -> RouteComparison:
    """
    Run the full agent pipeline and return a complete RouteComparison
    with emissions, costs, scoring, and agentic reasoning.
    """
    ...
    # --- Stage 2.5: Scoring Engine ---
    scoring = score_routes(options, priority)

    # Build ScoredRoute objects from scoring result dicts
    scored_routes = [ScoredRoute(**r) for r in scoring["routes"]]
    recommended_route = ScoredRoute(**scoring["recommended"]) if scoring["recommended"] else None

    # Determine recommended mode for the decision agent
    recommended_mode = recommended_route.mode if recommended_route else None
